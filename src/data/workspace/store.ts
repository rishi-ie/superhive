...
import { validateWorkspaceInput, pickNextWorkspaceAfterDelete } from '@/functions/workspaces';
...
export function createWorkspace(input: { name: string; description?: string }): Workspace | null {
  const validated = validateWorkspaceInput(input);
  if (!validated) return null;
  const ws = repo.create(validated);
...
export async function deleteWorkspace(id: string): Promise<boolean> {
  ...
  if (currentWorkspaceId === id) {
    ...
    currentWorkspaceId = pickNextWorkspaceAfterDelete(remaining);
  }
  return ok;
}
