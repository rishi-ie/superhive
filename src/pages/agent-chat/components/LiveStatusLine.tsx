...
export function WorkedHeader({
  ...
  return (
    <button
      ...
      className="flex w-full select-none items-center gap-1.5 border-b border-border/70 pb-3 text-left text-[15px] leading-6 text-[#9C9C9C] transition-colors hover:text-foreground"
    >
...
export function ActivityStatusLine({ status, live = false }: { status: ActivityStatus; live?: boolean }) {
  const StatusIcon = status.kind === 'thinking' ? BulbIcon : Loading03Icon
	if (!live && (status.kind === 'warning' || status.kind === 'error')) {
		return <div className={`select-none ${status.kind === 'error' ? 'text-destructive' : 'text-amber-500'}`}>{status.kind === 'error' ? '❌' : '⚠'} {status.pastLabel}</div>
	}

  if (!live && status.source && (status.source.kind === 'thinking' || status.source.kind === 'tool-call')) {
    return (
      <details className="group select-none border-0 font-sans text-[15px] leading-6 text-[#9C9C9C]">
  ...
  return (
    <div className="flex min-h-7 select-none items-center gap-2 border-0 font-sans text-[15px] leading-6 text-[#9C9C9C]" aria-live={live ? 'polite' : undefined} aria-label={live ? 'Current agent activity' : undefined}>
...
function AuditDetail({ status }: { status: ActivityStatus }) {
  if (status.source?.kind === 'thinking') {
    return (
      <div className="ml-4 mt-1 max-h-64 select-none overflow-auto whitespace-pre-wrap text-[15px] leading-6 text-muted-foreground/75">
  ...
  return (
    <pre className="ml-4 mt-2 max-h-64 select-none overflow-auto whitespace-pre-wrap rounded-md border border-border/60 bg-muted/30 p-3 text-xs leading-relaxed text-foreground/80">
      {`Tool: ${tool.toolName}\nTarget: ${tool.target ?? '—'}\nOutcome: ${tool.state}\n\n${result}`}
    </pre>
  )
}
