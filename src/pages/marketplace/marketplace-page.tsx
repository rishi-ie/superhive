	...
	return (
		<div className="flex h-full min-h-0 flex-col bg-background">
			...
			<main className="no-scrollbar min-h-0 flex-1 overflow-y-auto">
				<div className="mx-auto w-full max-w-5xl px-5 pb-10 pt-5 sm:px-8">
					<section className="max-w-3xl"><h1 className="text-3xl font-medium tracking-tight">{content.label}</h1><p className="mt-1.5 text-base text-muted-foreground">{content.description}</p><label className="mt-5 flex h-10 items-center gap-2 rounded-full border border-border bg-card/40 px-3 text-muted-foreground"><Icon icon={MagnifyingGlassIcon} className="size-4" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={`Search ${content.label.toLowerCase()}`} className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground" /></label></section>
					{activeSection !== 'agents' ? <><section className="mt-8 max-w-3xl"><div className="flex items-center justify-between border-b border-border/60 pb-2.5"><h2 className="text-base font-medium">Installed</h2><span className="text-xs text-muted-foreground">Global library</span></div><div className="flex min-h-14 flex-wrap items-center gap-2 py-3">{installed.length ? installed.map((item) => <button key={item.id} type="button" title={item.label} onClick={() => toggleDetails(item.id)} className="flex size-9 items-center justify-center rounded-lg border border-border/70 bg-card/40 text-[10px] text-muted-foreground">{item.label.slice(0, 1)}</button>) : <span className="text-xs text-muted-foreground">Nothing installed yet</span>}</div></section><section className="mt-4 flex max-w-3xl items-center justify-between gap-3"><div className="flex items-center gap-1 text-sm"><button type="button" onClick={() => setScope('public')} className={`rounded-lg px-2.5 py-1.5 ${scope === 'public' ? 'bg-secondary font-medium' : 'text-muted-foreground'}`}>Public</button><button type="button" onClick={() => setScope('personal')} className={`rounded-lg px-2.5 py-1.5 ${scope === 'personal' ? 'bg-secondary font-medium' : 'text-muted-foreground'}`}>Personal</button></div><label className="flex items-center gap-1.5 text-xs text-muted-foreground"><Icon icon={FunnelSimpleIcon} className="size-4" /><span className="sr-only">Category</span><select value={category} onChange={(event) => setCategory(event.target.value)} className="bg-transparent outline-none">{categories.map((entry) => <option key={entry}>{entry}</option>)}</select></label></section></> : null}
